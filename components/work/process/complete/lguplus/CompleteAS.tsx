/**
 * lguplus/CompleteAS - LGU+ 재판매 A/S(WRK_CD=03) 작업완료
 *
 * 전처리: 집선등록 필수 체크 + UPLS 작업완료 (bizType='02')
...
const LguplusCompleteAS: React.FC<CompleteComponentProps> = (props) => {
  const handlePreSubmit = useCallback(async (ctx: PreSubmitContext): Promise<PreSubmitResult | void> => {
    ...
    try {
      ...
      console.log('[LguplusCompleteAS] setUplsWorkComplete 완료');
    } catch (error: any) {
      console.error('[LguplusCompleteAS] UPLS 호출 에러:', error);
...
export default LguplusCompleteAS;
