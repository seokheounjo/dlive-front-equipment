...
const CompleteAS: React.FC<CompleteASProps> = ({
  ...
  useEffect(() => {
    ...
    localStorage.setItem(getStorageKey(), JSON.stringify(draftData));
      ...
      upCtrlCl, tvType, isDataLoaded, isWorkCompleted]);

  // 공통코드 로드
  useEffect(() => {
    const loadCodes = async () => {
      try {
        // CMCU005: 고객관계, CMCU057: 인터넷, CMCU110: VoIP, CMCU148: 디지털
        // CMWK009: KPI제품그룹, CMWK010: 처리유형, CMWK011: 처리유형상세
        // CMCT015: 상향제어 (레거시: ds_up_ctrl_cl)
        const codes = await getCommonCodeList([
          'CMCU005', 'CMCU057', 'CMCU110', 'CMCU148',
          'CMWK009', 'CMWK010', 'CMWK011', 'CMCT015'
        ]);
        ...
        if (codes['CMWK009']) {
          setKpiProdGrpOptions(codes['CMWK009'].map((code: CommonCode) => ({
            value: code.COMMON_CD, label: code.COMMON_CD_NM
          })));
        }
        if (codes['CMWK010']) {
          setObsRcptCdOptions(codes['CMWK010'].map((code: CommonCode) => ({
            value: code.COMMON_CD, label: code.COMMON_CD_NM
          })));
        }
        if (codes['CMWK011']) {
          setObsRcptDtlCdOptions(codes['CMWK011'].map((code: CommonCode) => ({
            value: code.COMMON_CD, label: code.COMMON_CD_NM
          })));
        }
        if (codes['CMCT015']) {
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-5">
          <div className="space-y-3 sm:space-y-5">
            ...
            <div>
              <label className="block text-xs sm:text-sm font-semibold text-gray-900 mb-1.5">
                제품그룹 {!isWorkCompleted && <span className="text-red-500">*</span>}
              </label>
              <Select value={kpiProdGrp} onValueChange={setKpiProdGrp} options={kpiProdGrpOptions}
                placeholder="제품그룹 선택" required disabled={isWorkCompleted} />
            </div>
            ...
            <div>
              ...
              <Select value={obsRcptDtlCd} onValueChange={setObsRcptDtlCd} options={obsRcptDtlCdOptions}
                placeholder="처리유형상세 선택" required disabled={isWorkCompleted} />
            </div>
