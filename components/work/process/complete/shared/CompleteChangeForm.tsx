...
import { getCommonCodeList, CommonCode, getCommonCodes, getWorkReceiptDetail, getCustomerContractInfo, sendSignal, getLghvProdMap, getOttSale, getProdPromotionInfo } from '../../../../../services/apiService';
...
interface CompleteChangeFormProps {
  order: WorkOrder;
  onBack: () => void;
  onSuccess: () => void;
  ...
  /** 상품유형별 전처리 (UPLS/CL-04 등) - wrapper에서 주입 */
...
const CompleteChangeForm: React.FC<CompleteChangeFormProps> = ({
  order,
  onBack,
  onSuccess,
  showToast,
  ...
  const { certifyRegconfInfo, certifyProdList } = useCertifyStore();
  ...
  const handleSubmit = () => {
    ...
    if (errors.length > 0) {
      ...
      const fieldMap: [string, string][] = [
        ...
        ['배선구분', 'field-installInfo'],
        ['설치구분', 'field-installInfo'],
        ['설치정보', 'field-installInfo'],
        ['설치위치', 'field-installLocation'],
        ['작업처리일', 'field-workDate'],
        ['사용사업자', 'field-useOperators'],
        ['인터넷 사용사업자', 'field-useOperators'],
        ['VoIP 사용사업자', 'field-useOperators'],
        ['DTV 사용사업자', 'field-useOperators'],
        ['신호처리', 'field-equipment'],
        ['장비등록', 'field-equipment'],
        ['OTT', 'field-ott'],
        ['결합', 'field-joinCtrt'],
      ];
  ...
  const handleConfirmSubmit = async () => {
    ...
    if (onPreSubmit) {
      try {
        const preResult = await onPreSubmit({ order, workerId, installedEquipments, removedEquipments, certifyRegconfInfo, prodPromoInfo: equipmentData?.prodPromoInfo || [] });
        if (preResult && !preResult.success) {
          ...
          // severity === 'warning': 경고만 표시하고 계속 진행
    ...
    const isCertifyProdForSignal = !sendSignalAfterComplete;
    ...
    // OLD_PROD_CD가 LGU+(LGCT001)인지 판별 (레거시 mowou03m05 line 2167)
    // LGU+ → 딜라이브 상변 시 구상품이 LGU+면 해지신호(SMR05)만 skip (완료 프로시져/배치 처리)
    // 개통신호(SMR03)는 2022.01.26 이후 LGU+ 체크 제거됨 → 항상 전송 (레거시 line 1509-1522 주석처리)
    const oldProdCdForUpls = oldProdCd;
    console.log('[CompleteChange] LGU+ 체크: OLD_PROD_CD=', oldProdCd);
    let isOldUpls = false;
    try {
      const uplsCodes = await getCommonCodes('LGCT001');
      isOldUpls = uplsCodes.some((item: any) => (item.code || item.COMMON_CD) === oldProdCdForUpls);
      if (isOldUpls) {
        console.log('[CompleteChange] 구상품이 LGU+ 상품: 해지신호+개통신호 skip (배치처리)');
      }
    } catch (e) {
      console.warn('[CompleteChange] LGCT001 조회 실패:', e);
    }
    ...
    if (isLghvStb && isOldLghv) {
      ...
      // 레거시 mowou03m05 line 2167-2185: OLD가 LGU+면 해지신호(SMR05) skip (완료 프로시져/배치 처리)
      console.log('[CompleteChange] OLD가 LGU+ 상품: 해지신호(SMR05) skip (레거시 동일)');
    } else if (!isCertifyProdForSignal && removedEquipments.length > 0) {
      // 레거시 line 1812: CERTIFY_TG!="Y" 일 때만 철거신호

      // 구상품이 FTTH 인증상품인지 확인 (방어 로직)
      let isOldFtth = false;
      try {
        const certifyProdList = await getCertifyProdMap();
        isOldFtth = (certifyProdList || []).some((cd: string) => cd === oldProdCd);
      } catch (e) {
        console.warn('[CompleteChange] getCertifyProdMap 실패:', e);
      }

      if (isOldFtth) {
        // FTTH 구상품 → CL-06 해지신호
        try {
          console.log(`[신호연동] 상품변경(WRK_CD=05) | 구상품(${oldProdCd}) FTTH → CL-06 해지신호 전송`);
          ...
          if (cl06Result && !cl06Result.ERROR) {
            console.log('[CompleteChange] CL-06 해지신호 완료');
          } else {
            console.warn('[CompleteChange] CL-06 해지신호 실패 (계속 진행):', cl06Result?.ERROR);
            await showWarning(`CL-06 해지신호 실패: ${cl06Result?.ERROR || '알 수 없는 오류'}`);
          }
        } catch (error) {
          console.warn('[CompleteChange] CL-06 해지신호 오류 (계속 진행):', error);
          await showWarning(`CL-06 해지신호 오류: ${(error as any)?.message || '알 수 없는 오류'}`);
        }
      } else {
      // 일반/LGHV 구상품 → SMR05 또는 STB_DEL
      try {
        ...
        const rmvProdDesc = msgId === 'STB_DEL' ? 'LGHV STB 삭제' : 'SMR05 철거';
        console.log(`[신호연동] 상품변경 작업완료(WRK_CD=05) | 구상품 철거 ${rmvProdDesc} 신호 전송 | 구계약 CTRT_ID=${oldCtrtIdForSignal} | PROD_CD=${order.PROD_CD}`, { msgId, eqtProdCmpsId });
    ...
    const completeData: WorkCompleteData = {
      workInfo: {
        ...
        ADD_ON: (equipmentData?.prodPromoInfo || [])
    ...
    const sendInstallationSignal = async () => {
      try {
        ...
        // 레거시 mowou03m05 line 1509-1522: 개통신호 LGU+ 체크는 2022.01.26에 주석처리됨
        // → 구상품이 LGU+여도 신상품 D-Live 개통신호(SMR03)는 항상 전송
        ...
        const instProdDesc = msgId === 'PKG_CHG' ? 'LGHV 패키지변경(L→L)' : msgId === 'STB_CRT' ? 'LGHV STB 생성(비L→L)' : msgId === 'SMR60' ? 'VoIP 개통' : 'SMR03 개통';
        console.log(`[신호연동] 상품변경 작업완료(WRK_CD=05) | 신상품 설치 ${instProdDesc} 신호 전송 | DTL_CTRT_ID=${order.DTL_CTRT_ID || order.CTRT_ID} | EQT_NO=${eqtNo}`, { msgId, instProdCd, instEqtProdCmpsId, subProdCd });
        await sendSignal({
          MSG_ID: msgId,
          CUST_ID: order.customer?.id || (order as any).CUST_ID || '',
          CTRT_ID: order.DTL_CTRT_ID || order.CTRT_ID || '',
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-5">
          <div className="space-y-3 sm:space-y-5">
            ...
            {/* 타사이용여부 */}
            <div id="field-useOperators" className="border border-gray-200 rounded-lg overflow-hidden">
              <div className="px-3 sm:px-4 py-2 sm:py-3 bg-gray-50">
                <span className="text-sm font-semibold text-gray-700">타사이용여부 {!isWorkCompleted && <span className="text-red-500">*</span>}</span>
              </div>
              <div className="p-4 space-y-4 bg-white">
                <div>
                  <label className="block text-xs font-semibold text-gray-900 mb-1">DTV (디지털이용)</label>
                  <Select value={dtvUse} onValueChange={setDtvUse} options={dtvOptions} placeholder="선택" disabled={isWorkCompleted} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-900 mb-1">ISP (인터넷이용)</label>
                  <Select value={internetUse} onValueChange={setInternetUse} options={internetOptions} placeholder="선택" disabled={isWorkCompleted} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-900 mb-1">VoIP (VoIP이용)</label>
                  <Select value={voipUse} onValueChange={setVoipUse} options={voipOptions} placeholder="선택" disabled={isWorkCompleted} />
                </div>
              </div>
            </div>
...
export default CompleteChangeForm;
