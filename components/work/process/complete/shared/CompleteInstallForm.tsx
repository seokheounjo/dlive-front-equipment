...
/**
 * CompleteInstallForm - 설치(WRK_CD=01) 작업완료 공유 폼 컴포넌트
 *
 * 레거시 참조: mowoa03m01.xml
 *
 * productType에 따라 전처리/신호 분기:
 * - basic: SMR03/STB_CRT 신호 전송
 * - ftth: CL-04 서비스 개통 등록, 신호 차단
 * - lguplus: UPLS 집선등록, 신호 차단
 *
 * basic/, ftth/, lguplus/ 래퍼에서 productType을 전달하여 사용
 */
...
interface CompleteInstallFormProps {
  order: WorkOrder;
  onBack: () => void;
  onSuccess: () => void;
...
const CompleteInstallForm: React.FC<CompleteInstallFormProps> = ({
  order,
  onBack,
  onSuccess,
  showToast,
  ...
  const handleConfirmSubmit = async () => {
    ...
    // FTTH(IS_CERTIFY_PROD=1) + SO가 인증대상(CMIF006)이면 신호 전송 안 함 (상위에서 계산)
    if (isCertifyProdForSignal) signalFlag = false;
    ...
    if (signalFlag && (installedEqList.length > 0 || hasIspProd)) {
      try {
        ...
        const prodTypeDesc = isLghvStb ? 'LGHV STB' : prodGrp === 'V' ? 'VoIP' : '딜라이브 일반';
        const signalDesc = msgId === 'STB_CRT' ? 'LGHV STB 생성' : msgId === 'SMR60' ? 'VoIP 개통' : 'SMR03 개통';
        console.log(`[신호연동] 설치 작업완료(WRK_CD=01) | 상품: ${prodTypeDesc} | PROD_CD=${order.PROD_CD} | ${signalDesc} 신호 전송 | EQT_NO=${eqtNo}, CTRT_ID=${order.CTRT_ID}`, { msgId, prodCd, subProdCd, etc1, etc2, etc3, etc4 });
    ...
    // prodPromoInfo: completeData.ADD_ON 용 (sendInstallationSignal 내부에서도 별도 조회하지만 스코프 밖)
    const prodPromoInfo = equipmentData?.prodPromoInfo || [];
          ...
          <div className="space-y-3 sm:space-y-5">
            ...
            <div id="field-useOperators" className="border border-gray-200 rounded-lg overflow-hidden">
              <div className="px-3 sm:px-4 py-2 sm:py-3 bg-gray-50">
                <span className="text-xs sm:text-sm font-semibold text-gray-700">타사이용여부 {!isWorkCompleted && <span className="text-red-500">*</span>}</span>
              </div>
            ...
            <div className="flex gap-1.5 sm:gap-2 pt-3 sm:pt-4 mt-3 sm:mt-4 border-t border-gray-200">
              ...
              {!isWorkCompleted && (
                <button
                  ...
                  disabled={isLoading || isProcessing || isOstChecking || isOstBlocked}
                  className={`flex-1 min-h-12 py-3 px-4 rounded-lg font-bold text-sm sm:text-base flex items-center justify-center gap-2 transition-colors ${
                    isLoading || isProcessing || isOstChecking || isOstBlocked
...
export default CompleteInstallForm;
