...
interface CompleteInternalMoveFormProps {
  order: WorkOrder;
  onBack: () => void;
  onSuccess: () => void;
  ...
  /** 상품유형별 전처리 (CL-04 등) - wrapper에서 주입 */
...
const CompleteInternalMoveForm: React.FC<CompleteInternalMoveFormProps> = ({
  order,
  onBack,
  onSuccess,
  showToast,
  ...
  const handleConfirmSubmit = async () => {
    ...
    if (removedEquipments.length > 0 && !isCertifyProdForSignal && !isLghvStb) {
      ...
      if (delSignalFlag) {
        ...
        const moveProdCd = (order as any).BASIC_PROD_CD || order.PROD_CD || '';
        let isMoveFtth = false;
        try {
          const certifyProdList = await getCertifyProdMap();
          isMoveFtth = (certifyProdList || []).some((cd: string) => cd === moveProdCd);
        } catch (e) {
          console.warn('[CompleteInternalMove] getCertifyProdMap 실패:', e);
        }

        if (isMoveFtth) {
          // FTTH defense-in-depth → CL-06 해지신호
          try {
            console.log(`[신호연동] 댁내이전(WRK_CD=06) | FTTH(${moveProdCd}) defense → CL-06 해지신호 전송`);
            ...
            if (cl06Result && !cl06Result.ERROR) {
              console.log('[CompleteInternalMove] CL-06 해지신호 완료 (defense)');
            } else {
              console.warn('[CompleteInternalMove] CL-06 실패:', cl06Result?.ERROR);
              await showWarning(`CL-06 해지신호 실패: ${cl06Result?.ERROR || '알 수 없는 오류'}`);
              return;
            }
          } catch (error) {
            console.error('[CompleteInternalMove] CL-06 오류:', error);
        ...
        try {
          ...
          console.log(`[신호연동] 댁내이전 작업완료(WRK_CD=06) | 구주소 철거 SMR05 신호 전송 | 철거장비 ${removedEquipments.length}개 | CTRT_ID=${order.CTRT_ID}`, { eqtProdCmpsId, voipJoinCtrtId });
          ...
          await showWarning('철거 신호 처리 중 오류가 발생했습니다.');
          return;
        }
        }
      }
    ...
    if (signalFlag) {
      try {
        ...
        const instProdDesc = isLghvStb ? 'LGHV STB' : prodGrp === 'V' ? 'VoIP' : '딜라이브 일반';
        const instSignalDesc = msgId === 'STB_CRT' ? 'LGHV STB 생성' : msgId === 'SMR60' ? 'VoIP 개통' : 'SMR03 개통';
        console.log(`[신호연동] 댁내이전 작업완료(WRK_CD=06) | 상품: ${instProdDesc} | 신주소 설치 ${instSignalDesc} 신호 전송 | EQT_NO=${eqtNo} | CTRT_ID=${order.CTRT_ID}`, { msgId, prodCd, subProdCd, etc1, etc2, etc3, etc4 });
...
export default CompleteInternalMoveForm;
