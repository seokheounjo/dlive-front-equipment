...
const CompleteASForm: React.FC<CompleteASFormProps> = ({
  order,
  onBack,
  onSuccess,
  showToast,
  ...
  const handleSubmit = () => {
    ...
    if (errors.length > 0) {
      ...
      const fieldMap: [string, string][] = [
        ...
        ['제품그룹', 'field-kpiProdGrp'],
        ['처리유형상세', 'field-obsRcptDtlCd'],
        ['처리유형', 'field-obsRcptCd'],
  ...
  const handleConfirmSubmit = async () => {
    ...
    if (onPreSubmit) {
      try {
        const preResult = await onPreSubmit({ order, workerId, installedEquipments, certifyRegconfInfo });
    ...
    // ftth/lguplus: 삭제신호 skip (sendSignalAfterComplete=false)
    if (isCertifyProdForSignal) {
      console.log('[CompleteAS] isCertifyProdForSignal=true - 삭제신호 skip (인증상품/LGU+)');
    } else if (isLghvStb) {
      ...
      // Defense-in-depth: FTTH 상품이 여기까지 올 경우 CL-06 전송
      const asProdCd = (order as any).BASIC_PROD_CD || order.PROD_CD || '';
      let isAsFtth = false;
      try {
        const certifyProdList = await getCertifyProdMap();
        isAsFtth = (certifyProdList || []).some((cd: string) => cd === asProdCd);
      } catch (e) {
        console.warn('[CompleteAS] getCertifyProdMap 실패:', e);
      }

      if (isAsFtth) {
        // FTTH defense-in-depth → CL-06 해지신호
        try {
          console.log(`[신호연동] A/S(WRK_CD=03) | FTTH(${asProdCd}) defense → CL-06 해지신호 전송`);
          ...
          if (cl06Result && !cl06Result.ERROR) {
            removalIfDtlId = cl06Result?.IF_DTL_ID || '';
            console.log('[CompleteAS] CL-06 해지신호 완료 (defense)');
          } else {
            console.error('[CompleteAS] CL-06 FAILED:', cl06Result?.ERROR);
            setIsProcessing(false);
            await showWarning(`CL-06 해지신호 실패: ${cl06Result?.ERROR || '알 수 없는 오류'}`);
            return;
          }
        } catch (error) {
          console.error('[CompleteAS] CL-06 error:', error);
          setIsProcessing(false);
          await showWarning('해지신호 전송 중 오류가 발생했습니다.');
          return;
        }
      } else {
      try {
        ...
        console.log(`[신호연동] A/S 작업완료(WRK_CD=03) | 상품: 딜라이브 일반 | PROD_CD=${order.PROD_CD} | 교체장비 철거 SMR05 신호 전송 | 철거장비 ${removedEquipments.length}개 | CTRT_ID=${order.CTRT_ID}`);
    ...
    if (isVoipIpc && removedEquipments.length > 0 && hasRealRemoval) {
      try {
        ...
        console.log(`[신호연동] A/S 작업완료(WRK_CD=03) | 상품: VoIP IPC (복합VoIP) | SMR65(IPC 전용) 신호 전송 | IPC장비 EQT_NO=${ipcEqtNo}`);
        ...
        if (smr65Result?.code === 'SUCCESS' || smr65Result?.code === 'PARTIAL') {
          ...
          setIsProcessing(false);
          await showWarning(`IPC 신호 전송 실패: ${errMsg}`);
          return;
        }
      } catch (error) {
        console.error('[CompleteAS] SMR65 error:', error);
        setIsProcessing(false);
        await showWarning('IPC 신호 전송 중 오류가 발생했습니다.');
        return;
      }
    }
    ...
    const completeData: WorkCompleteData = {
      workInfo: {
        ...
        ADD_ON: (equipmentData?.prodPromoInfo || [])
          .filter((item: any) => (item.PROD_CMPS_CL === '21' || item.PROD_CMPS_CL === '22') && item.PROD_STAT_CD === '10')
    ...
    const sendInstallationSignal = async () => {
      try {
        ...
        const instProdTypeDesc = isLghvStb ? 'LGHV STB' : prodGrp === 'V' ? 'VoIP' : '딜라이브 일반';
        const instSignalDesc = msgId === 'STB_CHG' ? 'LGHV STB 변경' : msgId === 'SMR60' ? 'VoIP 개통' : 'SMR03 개통';
        console.log(`[신호연동] A/S 작업완료(WRK_CD=03) | 상품: ${instProdTypeDesc} | 교체장비 설치 ${instSignalDesc} 신호 전송 | EQT_NO=${eqtNo}`, { msgId, instProdCd, instEqtProdCmpsId, subProdCd });
        await sendSignal({
          MSG_ID: msgId,
          CUST_ID: order.customer?.id || (order as any).CUST_ID || '',
          CTRT_ID: order.CTRT_ID || '',
