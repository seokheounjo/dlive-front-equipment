...
interface CompleteSuspendFormProps {
  order: WorkOrder;
  onBack: () => void;
  onSuccess: () => void;
...
const CompleteSuspendForm: React.FC<CompleteSuspendFormProps> = ({
  order,
  onBack,
  onSuccess,
  showToast,
  ...
  const handleConfirmSubmit = async () => {
    ...
    if (isDelsignalTarget && (removedEquipments.length > 0 || customerEquipmentsForDel.length > 0 || installedEquipmentsForDel.length > 0 || hasIspProdForDel)) {
      try {
        ...
        const prodTypeDesc = isLghvStb ? 'LGHV STB' : '딜라이브 일반';
        const signalDesc = msgId === 'STB_DEL' ? 'LGHV STB 삭제' : 'SMR05 철거';
        const wrkDtlDesc = {'0430':'일시철거','0440':'일시철거복구','0470':'직권정지철거','0480':'직권정지철거복구'}[(order as any).WRK_DTL_TCD] || (order as any).WRK_DTL_TCD;
        console.log(`[신호연동] 정지 작업완료(WRK_CD=04) | ${wrkDtlDesc} | 상품: ${prodTypeDesc} | ${signalDesc} 신호 전송 | CTRT_ID=${order.CTRT_ID}`, { msgId, eqtProdCmpsId });
        ...
        if (result.code !== 'SUCCESS' && result.code !== 'OK') {
          ...
          await showWarning(`철거 신호 전송에 실패했습니다: ${result.message || ''}`);
        } else {
    ...
    if (order.WRK_DTL_TCD === '0440' && (customerEquipments.length > 0 || installedEquipments.length > 0 || hasIspProd)) {
      try {
        ...
        const wrkDtlDescInst = {'0440':'일시철거복구','0480':'직권정지철거복구'}[(order as any).WRK_DTL_TCD] || (order as any).WRK_DTL_TCD;
        console.log(`[신호연동] 정지 작업완료(WRK_CD=04) | ${wrkDtlDescInst} | 상품: ${instProdDesc} | ${instSignalDesc} 신호 전송 | EQT_NO=${eqtNo} | CTRT_ID=${order.CTRT_ID}`, { msgId, prodCd, subProdCd, etc1, etc2, etc3, etc4 });
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          disabled={!!susError}
                          className="px-3 py-1 bg-indigo-500 hover:bg-indigo-600 text-white rounded text-xs font-medium disabled:bg-gray-400 disabled:text-white disabled:cursor-not-allowed"
                        >
                          확인
                        </button>
                      </div>
...
export default CompleteSuspendForm;
