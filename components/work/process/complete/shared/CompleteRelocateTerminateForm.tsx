...
import { getCommonCodeList, getCommonCodes, CommonCode, insertWorkRemoveStat, modAsPdaReceipt, getWorkReceiptDetail, sendSignal, getMoveWorkInfo, getLghvProdMap, getMmtSusInfo, modMmtSusInfo } from '../../../../../services/apiService';
...
/**
 * CompleteRelocateTerminateForm - 이전철거(WRK_CD=08) 작업완료 컴포넌트
 *
 ...
 * - 재사용 체크 (chk_reuse_yn)
 */
interface CompleteRelocateTerminateFormProps {
  order: WorkOrder;
  onBack: () => void;
  onSuccess: () => void;
...
const CompleteRelocateTerminateForm: React.FC<CompleteRelocateTerminateFormProps> = ({
  order,
  onBack,
  onSuccess,
  showToast,
  ...
  const equipmentData = {
    ...
    // 고객장비 (API output4, 레거시 ds_eqt_cust) - 신호조건 판단에 필요
    customerEquipments: zustandEquipment.customerEquipments.length > 0
      ? zustandEquipment.customerEquipments
      : (storeEquipmentData?.customerEquipments || legacyEquipmentData?.customerEquipments || []),
  ...
  const sendRemovalSignal = async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log(`[신호연동] 이전철거 작업완료(WRK_CD=08) | 상품: ${prodTypeDesc} | PROD_CD=${order.PROD_CD} | ${signalDesc} 신호 전송 | CTRT_ID=${order.CTRT_ID}`, {
        msgId, isLghvStb, eqtProdCmpsId, etc1, voipJoinCtrtId,
      });
  ...
  const handleSubmit = () => {
    ...
    if (errors.length > 0) {
      ...
      const fieldMap: [string, string][] = [
        ...
        ['배선방식', 'field-installInfo'],
        ['설치유형', 'field-installInfo'],
  ...
  const handleConfirmSubmit = async () => {
    ...
    // sendSignalAfterComplete=false → certifyTg 판별 (lguplus: 'Y' 고정, ftth: CL-08 기반)
    // sendSignalAfterComplete=true (basic) → certifyTg='N' (SMR05 발동, CL-06 SKIP)
    //
    // 레거시 certifyTg 판별 (mowoa03m08 line 477-493):
    //   IS_CERTIFY_PROD == 1 → CL-08 호출 → CONT_ID == DTL_CTRT_ID → 'Y'
    // certifyTg='Y' → SMR05 SKIP + CL-06 발동
    // certifyTg!='Y' → SMR05 발동 + CL-06 SKIP
    let certifyTg = 'N';
    if (!sendSignalAfterComplete && productType === 'lguplus') {
      certifyTg = 'Y';
    } else if (!sendSignalAfterComplete && productType === 'ftth' && ((order as any).IS_CERTIFY_PROD == 1 || (order as any).IS_CERTIFY_PROD === '1')) {
      // CL-08 호출로 인증 상태 확인 (레거시 fn_certify_cl08과 동일)
      try {
        const dtlCtrtId = (order as any).DTL_CTRT_ID || order.CTRT_ID || '';
        ...
        if (cl08Result && !cl08Result.ERROR && cl08Result.CONT_ID === dtlCtrtId) {
          certifyTg = 'Y';
          console.log('[CompleteRelocateTerminate] CL-08 인증 확인, CERTIFY_TG=Y (CONT_ID=DTL_CTRT_ID)');
        } else {
          console.log('[CompleteRelocateTerminate] CL-08 인증 미확인, CERTIFY_TG=N', cl08Result);
        }
      } catch (cl08Err) {
        console.log('[CompleteRelocateTerminate] CL-08 조회 실패 (CERTIFY_TG=N):', cl08Err);
      }
    }
    ...
    if (!signalSent && certifyTg !== 'Y' &&
      ...
      if (!signalSuccess) {
        ...
        if (prodGrp === 'V' && errMsg.indexOf('PROC_VOIP_KCT-029') < 0) {
          await showWarning('VoIP 신호 전송에 실패했습니다.');
          return;
        }
    ...
    // 단말인증(SO) CL-06 해지요청 (레거시 mowoa03m08 line 930-940)
    // certifyTg='Y' (CL-08에서 인증 확인됨) → CL-06 발동 조건 확인
    // OLD는 단말인증 대상이고, NEW는 일반상품이거나 전입지 SO가 인증 대상이 아닌 경우 → CL-06
    // 장비 유무와 무관하게 인증 상품이면 CL-06이 나가야 함
    if (certifyTg === 'Y' && productType !== 'lguplus') {
      try {
        // 전입지 NEW_PROD_CD, SO_ID (레거시: ds_move_info)
        const newProdCd = moveWorkInfoData?.NEW_PROD_CD || '';
        const newSoId = moveWorkInfoData?.SO_ID || '';

        // certify product list (이미 useEffect에서 로드됨)
        const prodList = certifyProdListState.length > 0
          ? certifyProdListState
          : await getCertifyProdMap();
        const isNewProdCertify = prodList.some((item: any) => item.PROD_CD === newProdCd);

        // certify SO list (CMIF006)
        ...
        console.log('[CompleteRelocateTerminate] CL-06 조건 확인:', {
          certifyTg, newProdCd, newSoId,
          isNewProdCertify, isNewSoCertify,
          shouldCallCL06: !isNewProdCertify || !isNewSoCertify,
        });

        // NEW가 인증상품이 아니거나, 전입지 SO가 인증 SO가 아닌 경우 → CL-06 해지요청
        if (!isNewProdCertify || !isNewSoCertify) {
          console.log('[CompleteRelocateTerminate] CL-06 해지요청 자동 호출');
          ...
          if (cl06Result?.ERROR) {
            showToast?.(`단말인증 해지요청에 실패\n${cl06Result.ERROR}`, 'error', true);
            return;
          }
          console.log('[CompleteRelocateTerminate] CL-06 해지요청 성공');
        }
      } catch (cl06Err: any) {
        console.error('[CompleteRelocateTerminate] CL-06 처리 오류:', cl06Err);
        showToast?.(`단말인증 해지요청에 실패: ${cl06Err.message || ''}`, 'error', true);
        return;
      }
    }
...
export default CompleteRelocateTerminateForm;
