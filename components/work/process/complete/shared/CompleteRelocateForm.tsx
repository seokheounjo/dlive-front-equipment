...
interface CompleteRelocateFormProps {
  order: WorkOrder;
  onBack: () => void;
  onSuccess: () => void;
...
const CompleteRelocateForm: React.FC<CompleteRelocateFormProps> = ({
  order,
  onBack,
  onSuccess,
  showToast,
  ...
  const handleConfirmSubmit = async () => {
    ...
    if (onPreSubmit) {
      try {
        const preResult = await onPreSubmit({ order, workerId, installedEquipments, removedEquipments, certifyRegconfInfo });
    ...
    if (!isCertifyProdForSignal && (hasIspProd || installedEqList.length > 0)) {
      try {
        ...
        const instProdDesc = msgId === 'STB_CRT' ? 'LGHV STB 생성' : msgId === 'SMR60' ? 'VoIP 개통' : 'SMR03 개통';
        console.log(`[신호연동] 이전설치 작업완료(WRK_CD=07) | 신주소 설치 ${instProdDesc} 신호 전송 | PROD_CD=${order.PROD_CD} | EQT_NO=${eqtNo} | CTRT_ID=${order.CTRT_ID}`, { msgId, prodCd, subProdCd, etc1, etc2, etc3, etc4 });
        ...
        if (signalResult.code === 'SUCCESS' || signalResult.code === 'OK') {
          ...
          if (prodGrp !== 'V') {
            await showWarning('개통 신호 전송이 실패했습니다. 계속 진행합니다.');
          }
        }
      } catch (error: any) {
    ...
    if (removedEquipments.length > 0 && !isCertifyProdForSignal && !isLghvStb) {
      ...
      if (delSignalFlag) {
        // Defense-in-depth: FTTH 상품 체크
        const relocProdCd = (order as any).BASIC_PROD_CD || order.PROD_CD || '';
        let isRelocFtth = false;
        try {
          const certifyProdList = await getCertifyProdMap();
          isRelocFtth = (certifyProdList || []).some((cd: string) => cd === relocProdCd);
        } catch (e) {
          console.warn('[CompleteRelocate] getCertifyProdMap 실패:', e);
        }

        if (isRelocFtth) {
          // FTTH defense-in-depth → CL-06 해지신호
          try {
            console.log(`[신호연동] 이전설치(WRK_CD=07) | FTTH(${relocProdCd}) defense → CL-06 해지신호 전송`);
            ...
            if (cl06Result && !cl06Result.ERROR) {
              console.log('[CompleteRelocate] CL-06 해지신호 완료 (defense)');
            } else {
              console.warn('[CompleteRelocate] CL-06 실패:', cl06Result?.ERROR);
              await showWarning(`CL-06 해지신호 실패: ${cl06Result?.ERROR || '알 수 없는 오류'}`);
              return;
            }
          } catch (error) {
            console.error('[CompleteRelocate] CL-06 오류:', error);
        ...
        try {
          ...
          console.log(`[신호연동] 이전설치 작업완료(WRK_CD=07) | 구주소 철거 SMR05 신호 전송 | 철거장비 ${removedEquipments.length}개 | CTRT_ID=${order.CTRT_ID}`, { delEqtProdCmpsId, prodGrpForDel });
...
export default CompleteRelocateForm;
