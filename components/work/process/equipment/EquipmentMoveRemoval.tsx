 ...
 * WRK_CD=06 (대내이전설치), WRK_CD=07 (이전설치) 장비정보 컴포넌트
 * 레거시 참조: mowoDivD05.xml, mowoa03m07.xml
 *
 * 주요 기능:
 * - 이전설치(06,07) 전용 컴포넌트
 * - WRK_CD=06 (대내이전설치): 특정 6개 장비 코드만 철거 가능
...
const EquipmentMoveRemoval: React.FC<EquipmentComponentProps> = ({
  ...
  // 회수 장비 목록 (고객으로부터 회수할 장비 - API output5)
  ...
  const loadEquipmentData = async (forceRefresh = false) => {
    try {
      ...
      if (preloadedApiData && !forceRefresh) {
        console.log('[장비관리-이전설치] Pre-loaded 데이터 사용 - API 호출 건너뜀!');
        ...
        if (savedDraft) {
          try {
            ...
            if (draftData.installedEquipments && draftData.installedEquipments.length > 0) {
              console.log('[장비관리-이전설치] localStorage에서 장비 복원:', draftData.installedEquipments.length, '개');
            ...
            console.warn('[장비관리-이전설치] localStorage 데이터 파싱 실패:', error);
          }
        }
      }
  ...
  const handleMarkForRemoval = () => {
    ...
    // 레거시 검증: 대내이전설치(06) 작업 시만 재사용 가능한 장비만 철거 가능
    // 이전설치(07)은 제한 없음 - 레거시 mowoDivD05.xml 참조
    if (workItem.WRK_CD === '06') {
      ...
      if (!isMoveInstallReusableEquipment(eqtClCd)) {
        showToast?.('대내이전설치 작업유형은 해당 장비를 철거하실 수 없습니다.', 'error');
        return;
      }
    }
  ...
  return (
    <div className="px-2 sm:px-4 py-4 sm:py-6 space-y-3 sm:space-y-4 bg-gray-50 pb-4">
      ...
      {/* 고객 장비 섹션 (철거 대상) */}
      {!isWorkCompleted && removeEquipments.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 border-b border-gray-100 gap-2">
            <h4 className="text-sm sm:text-base font-bold text-gray-900">
              고객 장비 (철거 대상)
          ...
          <div className="p-3 sm:p-4 space-y-2.5">
            {removeEquipments.map(equipment => {
              ...
              const eqtClCd = equipment.eqtClCd || equipment.EQT_CL_CD;
              const isReusable = isMoveInstallReusableEquipment(eqtClCd);
              ...
              return (
                ...
                >
                  <div className="space-y-2 sm:space-y-2.5">
                    ...
                    <div className="space-y-1 sm:space-y-1.5">
                      <div className="text-xs sm:text-sm text-gray-600">S/N: {equipment.serialNumber}</div>
                      {equipment.macAddress && (
                        <div className="text-xs sm:text-sm text-gray-600">MAC: {equipment.macAddress}</div>
                      )}
                    </div>
                    {!isReusable && workItem.WRK_CD === '06' && (
                  ...
                  {isMarked && (
                    <div className="absolute top-3 sm:top-4 right-3 sm:right-4 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-orange-500 text-white flex items-center justify-center text-sm sm:text-base font-bold">
                      ✓
                    </div>
                  )}
                </div>
