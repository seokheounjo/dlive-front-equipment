...
import { getTechnicianEquipments, updateEquipmentComposition, checkStbServerConnection, getEqtSoMoveInfo, getMVRemoveEqtInfo, getMoveWorkInfo, MoveWorkInfo, RemovalEquipmentInfo } from '../../../../../services/apiService';
...
const EquipmentRelocateForm: React.FC<EquipmentComponentProps> = ({
  workItem,
  onSave,
  onBack,
  showToast,
  ...
  // ProductTypeContext에서 상품유형 정보 가져오기
  const { isLguProd: isCertifyProd, isFtthProd, needsLineRegistration: isCertifyForLineReg, opLnkdCd: certifyOpLnkdCd } = useProductType();
  ...
  // Certify Store (LDAP 현황 조회용)
  const { entrNo: storeEntrNo } = useCertifyStore();

  // AP/ONT MAC 추출 (레거시 mowou04p07.xml line 363-372)
  const ldapApMac = useMemo(() => {
    const ap = installedEquipments.find(eq => (eq.actualEquipment?.itemMidCd || eq.actualEquipment?.ITEM_MID_CD) === '10');
    return ap?.actualEquipment?.macAddress || ap?.actualEquipment?.MAC_ADDRESS || '';
  }, [installedEquipments]);
  const ldapOntMac = useMemo(() => {
    const ont = installedEquipments.find(eq => (eq.actualEquipment?.itemMidCd || eq.actualEquipment?.ITEM_MID_CD) === '02');
    return ont?.actualEquipment?.macAddress || ont?.actualEquipment?.MAC_ADDRESS || '';
  }, [installedEquipments]);
  ...
  const [mvRemoveLoading, setMvRemoveLoading] = useState(false);

  // 이전작업 정보 (getMoveWorkInfo - OLD_CTRT_ID, OLD_SO_ID, OLD_MST_SO_ID 분실처리용)
  const [moveWorkInfoData, setMoveWorkInfoData] = useState<MoveWorkInfo | null>(null);
  ...
  // 이전철거 장비 + 이관완료 장비 로드 (탭 이동 시에도 항상 복원)
  const loadMvRemoveData = async () => {
    setMvRemoveLoading(true);
    ...
    if (custId && rcptId) {
      try {
        mvEqts = await getMVRemoveEqtInfo({ CUST_ID: custId, RCPT_ID: rcptId });
        console.log('[장비관리-이전설치] 이전철거 장비 복원:', mvEqts.length, '건');
        setMvRemoveEquipments(mvEqts);
        mvCount = mvEqts.length;
      } catch (err) {
        console.error('[장비관리-이전설치] 이전철거 장비 복원 실패:', err);
      }
    }
    // 이전작업 정보 조회 (분실처리 시 OLD_CTRT_ID, OLD_SO_ID, OLD_MST_SO_ID 필요)
    if (workItem.WRK_CD === '07' && !moveWorkInfoData) {
      try {
        const moveInfo = await getMoveWorkInfo({
          WRK_CD: workItem.WRK_CD || '07',
          WRK_ID: workItem.id || '',
          RCPT_ID: workItem.RCPT_ID || '',
        });
        console.log('[장비관리-이전설치] getMoveWorkInfo:', moveInfo);
        setMoveWorkInfoData(moveInfo);
      } catch (err) {
        console.error('[장비관리-이전설치] getMoveWorkInfo 실패:', err);
      }
    }
    if (workItem.WRK_CD === '07' && workItem.id) {
      try {
        ...
        // 분실/파손 처리된 장비도 처리완료로 카운트 (custEqtInfoDel은 EqtSoMove에 기록 안 됨)
        for (const eq of mvEqts) {
          const serno = eq.EQT_SERNO || '';
          if (serno && !transferredSerials.has(serno)) {
            const stat = (eq.EQT_STAT_CD || '').toLowerCase();
            if (stat === '분실' || stat === '파손' || stat === '회수') {
              transferredSerials.add(serno);
            }
          }
        }
        trCount = transferredSerials.size;
        if (transferredSerials.size > 0) {
          setTransferredEquipmentSerials(transferredSerials);
        }
      } catch (err) {}
    }
    // Store에 카운트 저장 (WorkProcessFlow step 검증용)
    setMvRemoveStats(workId, mvCount, trCount);
    setMvRemoveLoading(false);
  };

  // 초기 데이터 로드
  useEffect(() => {
    // 이미 데이터가 로드된 상태면 장비 목록은 건너뛰되, 이전철거 장비는 항상 로드
    const hasPreloadedData = !!preloadedApiData;
    if (isDataLoaded && hasPreloadedData && (installedEquipments.length > 0 || markedForRemoval.length > 0 || contractEquipments.length > 0)) {
      console.log('[장비관리-이전] 이미 데이터 로드됨 - 기존 데이터 유지, 이전철거 장비 복원');
      loadMvRemoveData();
      return;
    }
  ...
  // preloadedApiData 도착 시 기사재고가 비어있으면 자동 재로드
  useEffect(() => {
    if (preloadedApiData && isDataLoaded && technicianEquipments.length === 0) {
      console.log('[장비관리-이전설치] preloadedApiData 도착 - 기사재고 비어있어 재로드');
      loadEquipmentData();
    }
  }, [preloadedApiData]);
  ...
  const loadEquipmentData = async (forceRefresh = false) => {
    ...
    try {
      ...
      console.log('[장비관리-이전설치] WRKR_ID 디버그:', {
        ...
        '최종 WRKR_ID': user?.workerId || user?.userId || workItem.WRKR_ID || '',
        forceRefresh,
      });
      ...
      if (hasValidPreloadedData && !forceRefresh) {
        ...
        const requestPayload = {
          WRKR_ID: user.workerId || user.userId || workItem.WRKR_ID || '',
      ...
      if (workItem.WRK_CD === '07' && workItem.id) {
        getEqtSoMoveInfo({ WRK_ID: workItem.id }).then((moveInfoList: any[]) => {
          ...
          // 분실/파손/회수 처리된 장비도 처리완료로 카운트 (custEqtInfoDel은 EqtSoMove에 기록 안 됨)
          for (const eq of loadedMvEqts) {
            const serno = eq.EQT_SERNO || '';
            if (serno && !transferredSerials.has(serno)) {
              const stat = (eq.EQT_STAT_CD || '').toLowerCase();
              if (stat === '분실' || stat === '파손' || stat === '회수') {
          ...
          // Store에 카운트 저장 (WorkProcessFlow step 검증용)
          setMvRemoveStats(workId, loadedMvCount, transferredSerials.size);
          setMvRemoveLoading(false);
        }).catch(() => {
          // 이관 조회 실패해도 분실/파손 장비는 카운트
          let fallbackTrCount = 0;
          for (const eq of loadedMvEqts) {
            const stat = (eq.EQT_STAT_CD || '').toLowerCase();
            if (stat === '분실' || stat === '파손' || stat === '회수') {
              fallbackTrCount++;
          ...
          setMvRemoveStats(workId, loadedMvCount, fallbackTrCount);
          setMvRemoveLoading(false);
        });
      } else {
        // WRK_CD != '07'이어도 카운트 초기화
        setMvRemoveStats(workId, loadedMvCount, 0);
        setMvRemoveLoading(false);
      }
      ...
      storeSetDataLoaded(workId, true);
    } catch (error) {
  ...
  const handleSignalProcess = async () => {
    ...
    // 인증상품(FTTH/LGU+)이면 임시개통 차단
    if (isFtthProd || isCertifyProd) {
      console.log(`[임시개통] 인증상품 차단: isFtthProd=${isFtthProd}, isCertifyProd=${isCertifyProd}, PROD_CD=${workItem.PROD_CD}`);
      showToast?.('인증상품은 임시개통을 지원하지 않습니다. 작업완료 후 신호연동에서 처리해주세요.', 'info');
      return;
    }
    ...
    try {
      ...
      const prodType = isFtthProd ? 'FTTH 인증상품' : isCertifyProd ? (['F','FG','Z','ZG'].includes(certifyOpLnkdCd) ? 'LGU+ FTTH' : ['N','NG'].includes(certifyOpLnkdCd) ? 'LGU+ 광랜' : 'LGU+ 인증상품') : '딜라이브 일반';
      console.log(`[신호연동] 이전설치(WRK_CD=07) | 상품: ${prodType} | PROD_CD=${workItem.PROD_CD} | 임시개통 SMR03(개통신호) 전송 | CTRT_ID=${workItem.CTRT_ID}, WRK_ID=${workItem.id}, EQT_NO=${stbEqtNo}, MODEM=${modemEqtNo}`);
      ...
      console.log(`[임시개통] SMR03 checkStbServerConnection 응답:`, result);
  ...
  return (
    <div className="px-2 sm:px-4 py-4 sm:py-6 space-y-3 sm:space-y-4 bg-gray-50 pb-4">
        ...
        {isRefreshing && (
          <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-10 flex items-center justify-center rounded-xl">
            <div className="flex flex-col items-center gap-2">
              <RotateCcw className="w-8 h-8 text-primary-700 animate-spin" />
            ...
            <div className="flex items-center gap-2">
              <h4 className="text-sm sm:text-base font-bold text-gray-900">
                ...
                {(workItem.productName || workItem.PROD_NM) && (
                  <span className="font-normal text-primary-700 ml-1">({workItem.productName || workItem.PROD_NM})</span>
                )}
              </h4>
              <button
                ...
                className={`flex items-center gap-1 px-2 py-1 rounded-lg transition-all text-xs font-medium ${
                  isRefreshing || isWorkCompleted
                    ...
                    : 'text-primary-700 hover:text-primary-700 hover:bg-primary-50 active:scale-95'
                }`}
                title="장비 정보 새로고침"
              >
          ...
          <div className="grid grid-cols-4 gap-2">
            <button
              ...
              onClick={() => {
                ...
                if (installedEquipments.length > 0) {
                  showToast?.('등록된 장비를 해제한 후 변경 가능합니다.', 'warning');
                  return;
                }
            ...
            {/* 신호처리 / LDAP연동 버튼 (LGU+ 상품이면 LDAP연동, 아니면 임시개통) */}
            {(isCertifyProd) ? (
              <button
                className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-lg border-2 transition-all active:scale-95 min-h-[56px] ${
                  isLdapDone
                    ...
                    : ldapLoading
                      ...
                      : ldapBlocked
                        ...
                        : 'border-orange-400 bg-orange-100 text-orange-700 hover:bg-orange-200 hover:border-orange-500'
                }`}
                onClick={() => onLdapConnect?.()}
                disabled={ldapBlocked || ldapLoading || isWorkCompleted}
              >
                {ldapLoading ? (
                ...
                {ldapBlocked && <span className="text-[0.5rem] text-gray-400">(청약필요)</span>}
              </button>
            ) : (
              <button
                className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-lg border-2 transition-all active:scale-95 min-h-[56px] ${
                  isWorkCompleted || isFtthProd
                ...
                onClick={() => !isFtthProd && !isWorkCompleted && !isSignalProcessing && handleSignalProcess()}
                disabled={isWorkCompleted || isFtthProd || isSignalProcessing}
              >
                <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              >
                ...
                {!mvRemoveLoading && removalBadgeCount > 0 && (
            ...
            <button
              ...
              onClick={() => {
                (isCertifyProd ? setIsLdapQueryModalOpen : setIsIntegrationHistoryModalOpen)(true);
              }}
            >
        ...
        <div className="flex items-center justify-center gap-3 sm:gap-4">
          ...
          <button
            className={`flex flex-col items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-xl border-2 transition-all ${
              !selectedStock || installedEquipments.some(eq => eq.actualEquipment.id === selectedStock?.id)
                ? 'border-gray-200 bg-gray-50 text-gray-400 cursor-not-allowed'
                : 'border-primary-500 bg-primary-50 text-primary-700 hover:bg-primary-100 cursor-pointer active:scale-95'
            }`}
            onClick={() => {
              if (!selectedStock) return;
      ...
      {!isWorkCompleted && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 relative">
          ...
          {customerEquipments.length > 0 ? (
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  ...
                  {selectedContract && availableStock.length > 0 && (
                    <span className={`px-2 py-0.5 text-xs font-bold rounded-full ${
                      bottomTab === 'stock' ? 'bg-primary-500 text-white' : 'bg-gray-300 text-gray-600'
                    }`}>
            ...
            <div className="py-3 px-4 border-b-2 border-primary-500">
              <div className="flex items-center justify-center gap-2">
                <span className="text-sm font-semibold text-primary-700">기사재고장비</span>
                {selectedContract && availableStock.length > 0 && (
                  ...
                  {availableStock.map(stock => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {selectedStock?.id === stock.id && (
                          <div className="absolute top-3 sm:top-4 right-3 sm:right-4 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-primary-500 text-white flex items-center justify-center text-sm sm:text-base font-bold">
                            ✓
                          </div>
                        )}
                      </div>
      ...
      <EquipmentMoveModal
          ...
          oldCtrtId={moveWorkInfoData?.OLD_CTRT_ID}
          oldSoId={moveWorkInfoData?.OLD_SO_ID}
          oldMstSoId={moveWorkInfoData?.OLD_MST_SO_ID}
          ...
          onSuccess={(transferredSerials: string[]) => {
            ...
            const newTransferred = new Set([...transferredEquipmentSerials, ...transferredSerials]);
            setTransferredEquipmentSerials(newTransferred);
            // Store 카운트도 업데이트 (WorkProcessFlow step 검증용)
            setMvRemoveStats(workId, mvRemoveEquipments.length, newTransferred.size);
...
export default EquipmentRelocateForm;
