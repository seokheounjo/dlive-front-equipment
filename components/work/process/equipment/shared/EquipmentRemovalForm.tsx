...
const EquipmentRemovalForm: React.FC<EquipmentComponentProps> = ({
  workItem,
  onSave,
  onBack,
  showToast,
  ...
  useEffect(() => {
    if (preloadedApiData && isDataLoaded && technicianEquipments.length === 0) {
      console.log('[장비관리-부가상품] preloadedApiData 도착 - 기사재고 비어있어 재로드');
      loadEquipmentData();
    }
  }, [preloadedApiData]);
  ...
  const handleSignalProcess = async () => {
    ...
    try {
      ...
      console.log(`[신호연동] 부가상품(WRK_CD=09) | 상품: ${prodType} | PROD_CD=${workItem.PROD_CD} | 임시개통 SMR03(개통신호) 전송 | CTRT_ID=${workItem.CTRT_ID}, WRK_ID=${workItem.id}, EQT_NO=${stbEqtNo}, MODEM=${modemEqtNo}`);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 relative">
        ...
        <div className="p-3 sm:p-4 border-b border-gray-100">
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            {/* 분실처리 버튼 (disabled - 부가상품 작업에서는 사용 불가) */}
            <button
              ...
              disabled
              title="부가상품 작업에서는 분실처리를 사용할 수 없습니다"
            >
              <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default EquipmentRemovalForm;
