...
const EquipmentInstallForm: React.FC<EquipmentComponentProps> = ({
  workItem,
  onSave,
  onBack,
  showToast,
  ...
  useEffect(() => {
    ...
    if (isDataLoaded && hasPreloadedData && (installedEquipments.length > 0 || contractEquipments.length > 0)) {
  ...
  useEffect(() => {
    if (preloadedApiData && isDataLoaded && technicianEquipments.length === 0) {
      console.log('[장비관리-설치] preloadedApiData 도착 - 기사재고 비어있어 재로드');
      loadEquipmentData();
    }
  }, [preloadedApiData]);
  ...
  const handleSignalProcess = async () => {
    ...
    try {
      ...
      const signalDesc = msgId === 'SMR60' ? 'VoIP 개통신호' : msgId === 'SMR03' ? '개통신호' : `개통신호(${msgId})`;
      console.log(`[신호연동] 설치(WRK_CD=01) | 상품: ${prodType} | PROD_CD=${workItem.PROD_CD} | 임시개통 ${signalDesc} 전송 | CTRT_ID=${workItem.CTRT_ID}, WRK_ID=${workItem.id}, EQT_NO=${stbEqtNo || eqtNo}, MODEM=${modemEqtNo}`);
      ...
      console.log(`[임시개통] ${msgId} checkStbServerConnection 응답:`, result);
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            {(isCertifyProd) ? (
              ...
              >
                ...
                <span className="text-xs font-semibold">{isLdapDone ? 'LDAP완료' : ldapBlocked ? 'LDAP연동' : 'LDAP연동'}</span>
                {ldapBlocked && <span className="text-[0.5rem] text-gray-400">(청약필요)</span>}
              </button>
            ) : (
              <button
                className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-lg border-2 transition-all active:scale-95 min-h-[56px] ${
                  isWorkCompleted || isFtthProd || workItem.PROD_GRP === 'V'
                ...
                disabled={isWorkCompleted || isFtthProd}
              >
                <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {(isCertifyProd) ? (
              ...
              <button
                className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-lg border-2 transition-all active:scale-95 min-h-[56px] ${
                  isFtthProd || workItem.PROD_GRP === 'V'
                    ? 'border-gray-200 bg-gray-200 text-gray-400 cursor-not-allowed'
      ...
      {!isWorkCompleted && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 border-b border-gray-100 gap-2">
            <h4 className="text-sm sm:text-base font-bold text-gray-900">
              기사 재고 장비
              {selectedContract && <span className="text-primary-700"> ({selectedContract.type})</span>}
            </h4>
...
export default EquipmentInstallForm;
