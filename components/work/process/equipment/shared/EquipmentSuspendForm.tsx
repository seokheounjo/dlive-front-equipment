...
const EquipmentSuspendForm: React.FC<EquipmentComponentProps> = ({
  workItem,
  onSave,
  onBack,
  showToast,
  ...
  useEffect(() => {
    if (preloadedApiData && isDataLoaded && technicianEquipments.length === 0) {
      console.log('[장비관리-정지] preloadedApiData 도착 - 기사재고 비어있어 재로드');
      loadEquipmentData();
    }
  }, [preloadedApiData]);
  ...
  const handleSignalProcess = async () => {
    ...
    try {
      ...
      console.log(`[신호연동] 정지(WRK_CD=04) | 상품: ${prodType} | PROD_CD=${workItem.PROD_CD} | WRK_DTL_TCD=${workItem.WRK_DTL_TCD} | 임시개통 SMR03(개통신호) 전송 | CTRT_ID=${workItem.CTRT_ID}, WRK_ID=${workItem.id}, EQT_NO=${stbEqtNo}, MODEM=${modemEqtNo}`);
          ...
          <div className="grid grid-cols-4 gap-2">
            <button
              className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-lg border-2 transition-all active:scale-95 min-h-[56px] ${
                !isTemporaryRemovalRecovery || isWorkCompleted
              ...
              onClick={() => {
                if (!isTemporaryRemovalRecovery || isWorkCompleted) return;
                if (installedEquipments.length > 0) {
              ...
              disabled={!isTemporaryRemovalRecovery || isWorkCompleted}
            >
            ...
            {(isCertifyProd) ? (
              <button
                ...
                onClick={() => isTemporaryRemovalRecovery && onLdapConnect?.()}
                disabled={ldapBlocked || !isTemporaryRemovalRecovery || ldapLoading || isWorkCompleted}
              >
                {ldapLoading && isTemporaryRemovalRecovery ? (
              ...
              <button
                className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-lg border-2 transition-all active:scale-95 min-h-[56px] ${
                  !isTemporaryRemovalRecovery || isWorkCompleted || isFtthProd
                    ? 'border-gray-200 bg-gray-200 text-gray-400 cursor-not-allowed'
                ...
                onClick={() => !isFtthProd && isTemporaryRemovalRecovery && !isWorkCompleted && !isSignalProcessing && handleSignalProcess()}
                disabled={!isTemporaryRemovalRecovery || isWorkCompleted || isFtthProd || isSignalProcessing}
              >
                <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* 분실처리 버튼 (disabled - 정지 작업에서는 사용 불가) */}
            <button
              ...
              disabled
              title="정지 작업에서는 분실처리를 사용할 수 없습니다"
            >
              <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default EquipmentSuspendForm;
