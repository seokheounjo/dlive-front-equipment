...
const EquipmentChangeForm: React.FC<EquipmentComponentProps> = ({
  workItem,
  onSave,
  onBack,
  showToast,
  ...
  // Certify Store (LDAP 현황 조회용 + 인증상품 목록)
  const { entrNo: storeEntrNo, certifyProdList } = useCertifyStore();

  // 일반 FTTH 인증상품 여부 (LGU+ FTTH가 아니더라도 certifyProdList에 포함되면 FTTH 인증상품)
  const isCertifyTarget = !isCertifyProd && certifyProdList.length > 0 && certifyProdList.includes(workItem.PROD_CD || '');
  ...
  useEffect(() => {
    ...
    // 단, preloadedApiData가 없으면 (다른 경로로 진입) 항상 API 재호출
  ...
  useEffect(() => {
    if (preloadedApiData && isDataLoaded && technicianEquipments.length === 0) {
      console.log('[장비관리-상품변경] preloadedApiData 도착 - 기사재고 비어있어 재로드');
      loadEquipmentData();
    }
  }, [preloadedApiData]);
  ...
  const handleRegisterEquipment = () => {
    ...
    // 등록 후 해당 장비를 선택 상태로 유지 (바로 회수 가능하도록)
    storeSetSelectedStock(workId, actualEquipment);
      };
  ...
  const handleSignalProcess = async () => {
    ...
    try {
      ...
      console.log(`[신호연동] 상품변경(WRK_CD=05) | 상품: ${prodType} | PROD_CD=${workItem.PROD_CD} | 임시개통 SMR03(개통신호) 전송 | DTL_CTRT_ID=${ctrtIdForProductChange}, WRK_ID=${workItem.id}, EQT_NO=${stbEqtNo}, MODEM=${modemEqtNo}`);
        ...
        <div className="p-3 sm:p-4 border-b border-gray-100">
          <div className="flex items-center justify-between mb-3 gap-1">
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <h4 className="text-sm sm:text-base font-bold text-gray-900 truncate">
                고객 설치 장비
                {newProductName && (
                  <span className="font-normal text-primary-700 ml-1">({newProductName})</span>
                )}
              </h4>
              <button
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            {(isCertifyProd) ? (
              ...
              <button
                className={`flex flex-col items-center justify-center py-2.5 px-1 rounded-lg border-2 transition-all active:scale-95 min-h-[56px] ${
                  isWorkCompleted || isFtthProd || isCertifyTarget
                ...
                onClick={() => !isFtthProd && !isCertifyTarget && !isWorkCompleted && !isSignalProcessing && handleSignalProcess()}
                disabled={isWorkCompleted || isFtthProd || isCertifyTarget || isSignalProcessing}
              >
                <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* 분실처리 버튼 (disabled - 상품변경 작업에서는 사용 불가) */}
            <button
              className="flex flex-col items-center justify-center py-2.5 px-1 rounded-lg border-2 border-gray-200 bg-gray-200 text-gray-400 transition-all min-h-[56px] cursor-not-allowed"
              disabled
              title="상품변경 작업에서는 분실처리를 사용할 수 없습니다"
            >
              <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <button
            className={`flex flex-col items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-xl border-2 transition-all ${
              ...
              || (workItem.WRK_DTL_TCD === '0510' && !technicianEquipments.some(eq => eq.id === selectedStock?.id))
                ? 'border-gray-200 bg-gray-50 text-gray-400 cursor-not-allowed'
            ...
            onClick={() => {
              ...
              // 0510(상품변경설치): 기사재고 장비 등록취소만 허용, 고객장비 회수는 차단
              if (workItem.WRK_DTL_TCD === '0510' && !isFromTechStock) return;

              if (isFromTechStock) {
                // 기사재고장비에서 등록한 장비 → 등록 취소 (탭 제한 없이 허용)
                ...
                if (installedIndex >= 0) {
                  ...
                  setBottomTab('stock'); // 기사재고장비 탭으로 전환
                }
                return;
              }
            ...
            disabled={!selectedStock || !installedEquipments.some(eq => eq.actualEquipment.id === selectedStock?.id) || (workItem.WRK_DTL_TCD === '0510' && !technicianEquipments.some(eq => eq.id === selectedStock?.id))}
          >
