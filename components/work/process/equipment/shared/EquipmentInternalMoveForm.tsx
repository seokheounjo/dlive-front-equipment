/**
 * EquipmentInternalMove Component
 * WRK_CD=06 (댁내이전) 장비정보 컴포넌트
 * 레거시 참조: mowoDivD05.xml, mowoa03m06.xml
 *
 * 주요 기능:
 * - 댁내이전 전용 컴포넌트
 * - 특정 6개 장비 코드만 철거 가능, 분실처리만 (장비이관 없음)
 * - 철거한 장비를 재사용하여 설치 가능
 * - 설치 + 철거 기능 모두 포함
 * - 분실/파손 체크박스 처리
 * - 신호처리, 장비모델변경, 연동이력 기능
 ...
 * - 090251: 기가와이파이-GU
 *
 * 주의: 이전설치(WRK_CD=07)는 EquipmentRelocate.tsx 사용
 */
...
const EquipmentInternalMoveForm: React.FC<EquipmentComponentProps> = ({
  workItem,
  onSave,
  onBack,
  showToast,
  ...
  useEffect(() => {
    if (preloadedApiData && isDataLoaded && technicianEquipments.length === 0) {
      console.log('[장비관리-이전] preloadedApiData 도착 - 기사재고 비어있어 재로드');
      loadEquipmentData();
    }
  }, [preloadedApiData]);
  ...
  // 회수 버튼 - 댁내이전(06): 특정 장비만 철거 가능
  const handleMarkForRemoval = () => {
    if (!selectedStock) return;

    // 레거시 검증: 댁내이전(06) 작업 시 재사용 가능한 장비만 철거 가능
    // 레거시 mowoa03m06.xml 라인 1905-1918
    const eqtClCd = selectedStock.eqtClCd || selectedStock.EQT_CL_CD;
    if (!isMoveInstallReusableEquipment(eqtClCd)) {
      showToast?.('댁내이전 작업으로 장비 철거하실 수 없습니다.\n장비교체처리불가', 'error');
  ...
  const handleSignalProcess = async () => {
    ...
    try {
      ...
      console.log(`[신호연동] 댁내이전(WRK_CD=06) | 상품: ${prodType} | PROD_CD=${workItem.PROD_CD} | 임시개통 SMR03(개통신호) 전송 | CTRT_ID=${workItem.CTRT_ID}, WRK_ID=${workItem.id}, EQT_NO=${stbEqtNo}, MODEM=${modemEqtNo}`);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.error('[장비관리-댁내이전] 새로고침 실패:', error);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 relative">
        ...
        <div className="p-3 sm:p-4 border-b border-gray-100">
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            {/* 분실처리 버튼 (disabled - 댁내이전 작업에서는 사용 불가) */}
            <button
              ...
              disabled
              title="댁내이전 작업에서는 분실처리를 사용할 수 없습니다"
            >
              <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default EquipmentInternalMoveForm;
