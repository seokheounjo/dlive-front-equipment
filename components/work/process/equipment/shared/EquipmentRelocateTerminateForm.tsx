...
const EquipmentRelocateTerminateForm: React.FC<EquipmentComponentProps> = ({
  workItem,
  onSave,
  onBack,
  showToast,
  ...
  const checkReuseCondition = async (autoSet: boolean) => {
    try {
      ...
      if (autoSet) {
        if (shouldShow) {
          // 조건 충족: 기본값 체크 해제 (초기값 unchecked)
          storeSetReuseAll(workId, false);
          console.log('[장비관리-이전철거] 전체재사용 자동 설정: false');
        } else {
  ...
  const loadEquipmentData = async (forceRefresh = false) => {
    try {
      ...
      // output4: 고객 설치 장비 (레거시 ds_eqt_cust - 신호조건 판단용)
      const customers: ExtendedEquipment[] = (apiResponse.customerEquipments || []).map((eq: any) => ({
        ...eq,
        id: eq.EQT_NO,
        type: eq.ITEM_MID_NM,
        model: eq.EQT_CL_NM,
      ...
      console.log('  - 고객장비:', customers.length, '개');
      console.log('  - 철거 대상:', removed.length, '개');

      // Store에 API 데이터 저장 (customerEquipments도 저장 - 신호조건 판단용)
      setApiData(workId, {
        customerEquipments: customers,
        ...
        <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            {showReuseCheckbox && !isWorkCompleted && !readOnly && removeEquipments.length > 0 && (
              <label className="flex items-center gap-1.5 px-2 py-1 rounded-lg cursor-pointer hover:bg-primary-50 active:bg-primary-100">
                <input
                  type="checkbox"
                  checked={reuseAll}
                  onChange={handleReuseAllChange}
                  className="w-4 h-4 rounded border-primary-300 text-primary-700 focus:ring-primary-500"
                />
                <span className="text-xs text-primary-600 font-medium">재사용</span>
              </label>
            )}
          </div>
                ...
                >
                  ...
                  {!isWorkCompleted && !readOnly && (
                    <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-100">
                      {/* 분실/파손 체크박스 */}
                      <label className={`flex items-center gap-1.5 px-2 py-1.5 rounded-lg whitespace-nowrap ${isCustomerOwned ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'cursor-pointer active:bg-gray-100'}`}>
                        <input
                          type="checkbox"
...
export default EquipmentRelocateTerminateForm;
