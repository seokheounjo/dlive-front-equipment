...
import { ArrowUp, ArrowDown, CheckCircle2, XCircle, Loader2, ScanBarcode, History, AlertTriangle, RotateCcw } from 'lucide-react';
...
const EquipmentASForm: React.FC<EquipmentComponentProps> = ({
  workItem,
  onSave,
  onBack,
  showToast,
  ...
  useEffect(() => {
    if (preloadedApiData && isDataLoaded && technicianEquipments.length === 0) {
      console.log('[장비관리-AS] preloadedApiData 도착 - 기사재고 비어있어 재로드');
      loadEquipmentData();
    }
  }, [preloadedApiData]);
  ...
  const handleSignalProcess = async () => {
    ...
    try {
      ...
      console.log(`[신호연동] A/S(WRK_CD=03) | 상품: ${prodType} | PROD_CD=${workItem.PROD_CD} | 임시개통 SMR03(개통신호) 전송 | CTRT_ID=${workItem.CTRT_ID}, WRK_ID=${workItem.id}, EQT_NO=${stbEqtNo}, MODEM=${modemEqtNo}`);
      ...
      if (result.MSGCODE === 'SUCCESS') {
        setSignalResult(workId, `임시개통 완료\n\n결과: ${result.MESSAGE || '성공'}`);
        setSignalStatus(workId, 'success');
      } else {
        setSignalResult(workId, `임시개통 실패\n\n${result.MESSAGE || '알 수 없는 오류'}`);
        setSignalStatus(workId, 'fail');
      }
    } catch (error: any) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.error('[장비관리-AS] 새로고침 실패:', error);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 relative">
        ...
        <div className="p-3 sm:p-4 border-b border-gray-100">
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            {/* 분실처리 버튼 (disabled - A/S는 장비별 회수/분실 사용) */}
            <button
              ...
              disabled
              title="A/S 작업은 장비별 회수/분실처리를 사용합니다"
            >
              <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default EquipmentASForm;
