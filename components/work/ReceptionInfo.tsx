  ...
  return (
    <div className="px-4 py-6 space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <h4 className="text-base font-bold text-gray-900 mb-4">접수 기본 정보</h4>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <span className="text-xs text-gray-500">접수 ID</span>
              <p className="text-sm font-medium text-gray-900">{workItem.RCPT_ID || '-'}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs text-gray-500">작업 ID</span>
              <p className="text-sm font-medium text-gray-900">{workItem.id}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs text-gray-500">작업 유형</span>
              <p className="text-sm font-medium text-blue-600">{workItem.typeDisplay}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs text-gray-500">작업 상태</span>
              <p className="text-sm font-medium text-gray-900">{getStatusText(workItem.WRK_STAT_CD)}</p>
            </div>
          </div>
          <div className="pt-2 border-t border-gray-100">
            <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
              <span className="text-sm text-blue-700 font-medium">예정 일시</span>
              <span className="text-sm font-semibold text-blue-900">{formatDateTime(workItem.scheduledAt)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* 작업 요청 상세 */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <h4 className="text-base font-bold text-gray-900 mb-3">작업 요청 상세</h4>
        <div className="p-4 bg-gray-50 rounded-lg text-sm text-gray-700 leading-relaxed min-h-[80px]">
      ...
      {workItem.installLocation && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <h4 className="text-base font-bold text-gray-900 mb-3">설치 위치</h4>
          <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
            <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            <span className="text-sm font-medium text-gray-900">{workItem.installLocation}</span>
          </div>
        </div>
      )}
      ...
      {workItem.cellNo && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <h4 className="text-base font-bold text-gray-900 mb-3">고객 연락처</h4>
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            <span className="text-sm font-medium text-gray-900">{workItem.cellNo}</span>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h4 className="text-base font-bold text-gray-900">문자발송이력</h4>
            {smsLoading ? (
          ...
          <svg
            className={`w-5 h-5 text-gray-400 transition-transform ${smsHistoryOpen ? 'rotate-180' : ''}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
                  ...
                  <div key={index} className="p-4 bg-gray-50 rounded-lg border border-gray-100">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-gray-900">{item.EML_SMS_SND_TP_NM || '-'}</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-2">
                      <div>수신: {item.CELL_PHN || '-'}</div>
                      <div>발신: {item.SMS_RCV_NO || '-'}</div>
                      {item.SEND_TIME && <div className="col-span-2">전송시간: {item.SEND_TIME}</div>}
                      {item.REG_NM && <div className="col-span-2">등록자: {item.REG_NM}</div>}
                    </div>
                    {item.MESSAGE && (
                      <div className="mt-2 p-2 bg-white rounded border border-gray-200 text-sm text-gray-700 whitespace-pre-wrap">
                        {item.MESSAGE}
                      </div>
                    )}
                  </div>
